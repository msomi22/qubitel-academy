...
function completeCountingOptions(options = []) {
  const nextOptions = [...options];
  const used = new Set(nextOptions);

  for (let number = 1; nextOptions.length < 4 && number <= 12; number += 1) {
...
const questions = items.map(([name, icon, count, options], index) => {
  const expandedOptions = completeCountingOptions(options);
  ...
  return defineMcqProblem({
    ...
    optionVisuals: expandedOptions.map((option) => textVisual(option)),
    readAloud: true,
    autoReadAloud: false,
